...
class AlamoFIreAddUserViewController: UIViewController {
    ...
    private func registerUser(user: UserRegister) {
        let parameter = ["email": user.email,
                         "password": user.password
                        ]
        ApiManagerAF.shared.callAPI(urlString: ApiManagerAF.shared.registerUrl, method: .post, parameter: parameter) { [weak self]
