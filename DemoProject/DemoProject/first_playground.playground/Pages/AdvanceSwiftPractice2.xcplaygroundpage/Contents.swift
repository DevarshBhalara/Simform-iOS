...
let manObj1 = Man()
/**createing object using multiple inits **/
...
var objTestDeinit : testDeinit? = testDeinit()
var d2 = objTestDeinit
d2 = nil
objTestDeinit = nil
...
if let temp3 = dev.residence?.numberOfRooms {
    print("Rooms are \(temp3)")
}else {
...
class Series2 {
    ...
    init(cast: [String], detail: SeriesProtocol){
        self.cast = cast
        self.details = detail
    }
...
var objSeries3 = Series2(cast: ["1","2","3"], detail: SeriesCheck(name: "Elite", seasonCount: 6, rating: 8.3))
objSeries3.prinntD()
...
class LinearCongruentialGenerator: RandomNumberGenerator {
    ...
    func random() -> Double {
        lastRandom = ((lastRandom * varA + varC)
            .truncatingRemainder(dividingBy: varM))
        return lastRandom / varM
    }
...
class Calculator{
    ...
    func calculatePerform(calVar: CalculatorVariable) {
        ...
        case .Addition:
            ...
            guard let result = delegate?.add(calVar: calVar) else {
                return
            }
            print(result)
            
        case .Subtraction:
            guard let result = delegate?.sub(calVar: calVar) else {
                return
            }
            print(result)
        case .Multiplication:
            guard let result = delegate?.mul(calVar: calVar) else {
                return
            }
            print(result)
        case .Division:
            guard let result = delegate?.div(calVar: calVar) else {
                return
            }
            print(result)
...
if case .one(let i) = nameeee {
    print(i)
}

/**Another way to do this **/
//guard case .one(let i) = nameeee else {
//    print("No")
//    return
//}
...
myClosure() //1
counter += 1
myClosure() //1 reference is still in memory
...
instance.closure()  //1  after execution reference will be nil
instance.counter += 1
instance.closure()  //2
